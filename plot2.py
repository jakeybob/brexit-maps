...
# fig, (ax1, ax2) = plt.subplots(1, 2, sharey=True, frameon=False, figsize=(20,6))
# fig.suptitle('Overall Title')

fig = plt.figure(frameon=False, figsize=(20, 6))
# fig.suptitle('Overall Title')
gs = gridspec.GridSpec(1, 3, width_ratios=[1, 0.25, 1])
gs.update(wspace=0.025, hspace=0.05)

ax1 = fig.add_subplot(gs[0])
axm = fig.add_subplot(gs[1], sharey=ax1, frameon=False)
ax2 = fig.add_subplot(gs[2], sharey=ax1)


for i, entry in enumerate(list(reversed(area))):
    axm.text(0.5, i+1, entry, ha='center', va='center', color='black', weight='bold', fontsize=20)
axm.set_xticks([])
axm.set_yticks([])
axm.tick_params(
...
legend = ax2.legend(handles=legendHandles, bbox_to_anchor=(1.03, 0.7825, 0.3, 0.5), loc=3)



# plt.show()
plt.savefig('pics/test7.png', alpha=True, dpi=300, bbox_inches='tight', pad_inches=0.5, transparent=False, facecolor='white')
#################################################################
...
# fig, (ax1, ax2) = plt.subplots(1, 2, sharey=False, frameon=False, figsize=(20,6))
# colours = [mapMisc.viridisBlue, mapMisc.viridisYellow]
# patch_handles = []
# left = np.zeros(len(area))
# y_pos = np.arange(len(area), 0, -1)
...
# for i, entry in enumerate(area):
#     area[i] = area[i] + '   .'
# ax2.set_yticklabels(area, weight='bold', fontsize=15)
#
# ax2.tick_params(
#     axis='y',          # changes apply to the x-axis
#     which='both',      # both major and minor ticks are affected
